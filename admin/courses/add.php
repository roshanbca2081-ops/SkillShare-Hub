...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($title)) {
        ...
        if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
            $image_path = upload_image($_FILES['image'], $target_dir);
            if (!$image_path) {
                $error = 'Failed to upload image. Allowed: jpg, jpeg, png, gif, svg, webp';
            }
        }

        if (empty($error)) {
            if (create_course_full($title, $slug, $description, $image_path, $duration, $difficulty_level, $rating, $enrolled_students, $mentor_name, null, $field_id)) {
                redirect('admin/courses/index.php');
            } else {
                $error = 'Failed to create course. The slug may already exist.';
            }
        }
    }
}
      ...
      <div class="app-layout">
        ...
        <div>
          <div class="page-title">
            <div>
              <h1>Add Course</h1>
              <p>Create a new course with image, mentor, and field assignment.</p>
            </div>
          ...
          <div class="card p-4" style="max-width:720px;">
            ...
            <form method="post" enctype="multipart/form-data">
              <div class="mb-3">
                ...
                <input type="text" name="title" class="form-control" value="<?php echo e($_POST['title'] ?? ''); ?>" required placeholder="e.g. Full Stack Web Development" />
              </div>

              <div class="mb-3">
                <label class="form-label">Slug</label>
                <input type="text" name="slug" class="form-control" value="<?php echo e($_POST['slug'] ?? ''); ?>" placeholder="Auto-generated if empty" />
                <div class="small text-light-emphasis mt-1">Auto-generated from title if left empty.</div>
              </div>

              <div class="mb-3">
                <label class="form-label">Description</label>
                <textarea name="description" class="form-control" rows="4" placeholder="Course description..."><?php echo e($_POST['description'] ?? ''); ?></textarea>
              </div>
                  ...
                  <div class="mb-3">
                    ...
                    <select name="field_id" class="form-select">
                      ...
                      <?php foreach ($fields as $f): ?>
                        <option value="<?php echo (int)$f['id']; ?>" <?php echo (!empty($_POST['field_id']) && (int)$_POST['field_id'] === (int)$f['id']) ? 'selected' : ''; ?>>
                          <?php echo e($f['name']); ?>
                        </option>
                      <?php endforeach; ?>
                    </select>
              ...
              <div class="row g-3">
                <div class="col-md-6">
                  <div class="mb-3">
                    ...
                    <input type="text" name="duration" class="form-control" value="<?php echo e($_POST['duration'] ?? ''); ?>" placeholder="e.g. 10 weeks" />
                  </div>
                </div>
                <div class="col-md-3">
                  <div class="mb-3">
                    <label class="form-label">Rating (0-5)</label>
                    <input type="number" name="rating" class="form-control" value="<?php echo e($_POST['rating'] ?? '0'); ?>" step="0.1" min="0" max="5" />
                  </div>
                </div>
                <div class="col-md-3">
                  <div class="mb-3">
                    <label class="form-label">Enrolled Students</label>
                    <input type="number" name="enrolled_students" class="form-control" value="<?php echo e($_POST['enrolled_students'] ?? '0'); ?>" min="0" />
                  </div>
                </div>
              </div>
              ...
              <div class="mb-3">
                ...
                <input type="text" name="mentor_name" class="form-control" value="<?php echo e($_POST['mentor_name'] ?? ''); ?>" placeholder="e.g. Prof. Nirmal Bhatia" />
              </div>

              <div class="mb-3">
                <label class="form-label">Course Image</label>
                <input type="file" name="image" class="form-control" accept="image/jpeg,image/png,image/gif,image/webp" />
                <div class="small text-light-emphasis mt-1">Recommended: 900x500px. Allowed: jpg, png, gif, webp</div>
              </div>

              <div class="d-flex gap-2 mt-4">
                <button class="btn btn--primary" type="submit"><i class="fa-solid fa-save"></i> Create Course</button>
